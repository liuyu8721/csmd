...
|----|----|----|
|1|Human reference genome (hg19)|http://genome.ucsc.edu/|
|2|Three assembled human genomes available on NCBI (hs_alt_Celera, hs_alt_HuRef, hs_ref_GRCh37)|http://www.htslib.org/|
...
* Phase I: Subtract reads from standard human reference genome (hg19)
* Phase II: Subtract reads from three additional assembled human genomes (AHG), including hs_alt_Celera, hs_alt_HuRef and hs_ref_GRCh37
...
```
### 4.3 Databases
CSMD will work with a series of libraries listed in Table 2, including human-related genomes or sequences (21G) and all RefSeq bacteria genomes (150G, as of November 2018). The build process will then require approximately 500GB of additional disk space and 200GB of RAM. These genomes or sequences can be found in DBPATH/hg19/SEQ, DBPATH/AHG/SEQ, DBPATH/EHG/SEQ and DBPATH/RefSeq/bacteria/SEQ, respectively. And the indexed files will be saved in DBPATH/hg19, DBPATH/AHG, DBPATH/EHG and DBPATH/RefSeq/bacteria, respectively.
```shell
csmd --download-library libname --db DBPATH
```
NOTE:<BR/>
--download-library libname: Permissible libname includes “hg19”, “AHG”, “EHG”, “nt”, or “RefSeqBac”.
       --db DBPATH: the store path for the download.
```shell
csmd --build-library libname --db DBPATH
```
NOTE: <BR/>
--build-library: Permissible libname includes “hg19”, “AHG”, “EHG”, “nt”, or “RefSeqBac”. This command generates csmd needed indexed databases, BWA index for hg19 and AHG databases, Bowtie2 index for EHG and RefSeq representative species databases, and blast index for nt and representative genomes databases. To obtain RepBase, go to http://www.girinst.org.<BR/>
...
```
csmd --cs hg19Removal --ref REFNAME --thread NUMBER --r1 SAMPLE.R1.fastq --r2 SAMPLE.R2.fastq --output SAMPLE.hg19removal.fastq
csmd --cs ahgRemoval --ref REFNAME --thread NUMBER --input SAMPLE.hg19removal.fastq --output SAMPLE.hg19removal.AHGremoval.fastq
csmd --cs lcrRemoval --ref REFNAME --thread NUMBER --input SAMPLE.hg19removal.AHGremoval.fastq --output SAMPLE.hg19removal.AHGremoval.LCRremoval.fasta
csmd --cs ehgRemoval --ref REFNAME --thread NUMBER \
--input SAMPLE.hg19removal.AHGremoval.LCRremoval.fasta --output SAMPLE.hg19Removal.AHGRemoval.LCRRemoval.EHGRemoval.fasta

```
NOTE:<BR/>
--cs: cs step detail name. Permissible step name includes “hg19Removal”, “ahgRemoval”, “lcrRemoval”, or “ehgRemoval”.<BR/>
...
--input SAMPLE.hg19Removal.AHGRemoval.LCRRemoval.EHGRemoval.fasta --sam SAMPLE.csmd.sam –-report SAMPLE.csmd.profile.report
```
