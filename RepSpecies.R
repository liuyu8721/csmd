...
species.taxid <- x$species_taxid[!duplicated(x$species_taxid)]
RepSpecies <- NULL
for (i in 1:length(species.taxid)) {
  y <- x[x$species_taxid==species.taxid[i],]
  if (nrow(y)==1) RepSpecies <- rbind(RepSpecies, y)
  else {
    y$L1 <- NA
    ...
    RepSpecies <- rbind(RepSpecies, y[1,-which(names(y) %in% c("L1", "L2"))])
  }
}

write.csv(RepSpecies, "SpeciesRepresentative.csv", quote = F, row.names = F)
